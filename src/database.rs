...
/// A wrapper around a `Database` instance. This exists to allow orderly cleanup
/// of the `Database` by signalling the background purge task to shut down when
/// this struct is dropped.
#[derive(Debug)]
pub(crate) struct DatabaseDropGuard {
    /// The `Database` instance that will be shut down when this `DatabaseHolder` struct
    /// is dropped.
    database: Database,
}
...
///
/// `Database` contains a `HashMap` storing the key/value data and all
/// `broadcast::Sender` values for active pub/sub channels.
///
/// A `Database` instance is a handle to shared state. Cloning `Database` is shallow and
/// only incurs an atomic ref count increment.
///
/// When a `Database` value is created, a background task is spawned. This task is
/// used to expire values after the requested duration has elapsed. The task
/// runs until all instances of `Database` are dropped, at which point the task
/// terminates.
#[derive(Debug, Clone)]
...
struct State {
    ...
    /// True when the Database instance is shutting down. This happens when all `Database`
...
struct Entry {
    ...
    /// Duration for entry insert next cycle
...
impl Drop for DatabaseDropGuard {
    fn drop(&mut self) {
        // Signal the 'Database' instance to shut down the task that purges expired keys
        self.database.shutdown_purge_task();
    }
}
...
impl Database {
    ...
    pub(crate) fn new() -> Database {
        ...
        let shared = Arc::new(Shared {
            state: Mutex::new(State {
                entries: HashMap::new(),
                //pub_sub: HashMap::new(),
                pool: SqlitePool::connect_lazy(&url).unwrap(),
        ...
        tokio::spawn(insert_expired_tasks(shared.clone()));
    ...
    pub(crate) fn set(&self, key: String, value: Bytes, period: Option<Duration>) {
        ...
        let _expires_at = period.map(|duration| {
            // `Instant` at which the key period.
...
impl Shared {
    ...
    fn insert_expired_keys(&self) -> Option<Instant> {
        ...
        // TODO, use clone() to resolve the borrow checker error, but not good
        while let Some((&(when, id), key)) = state.expirations.clone().iter().next() {
            if when > now {
...
async fn insert_expired_tasks(shared: Arc<Shared>) {
    ...
    debug!("Insert background task shut down")
}
